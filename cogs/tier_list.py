...
Items (and their image(s)) are added one at a time in a chat
conversation after /tierlist is run (see
utils/image_collect.collect_named_options).

The main results message shows a captioned overview grid (one
thumbnail per item, via Pillow), built from each item's first image.
When someone picks an item to tier it, they see ALL of that item's
images as separate embeds (Discord allows several embeds per message,
so no compositing needed there).
"""
...
TIERS = ["S", "A", "B", "C", "D", "F"]
MAX_ITEMS = 25  # Discord's dropdown option limit
...
async def _fetch_thumbnails(item_list: list[dict]) -> dict:
    """Downloads each item's first image (for the overview grid), by URL,
    since the conversational collection flow only hands back URLs."""
    thumb_bytes = {}
    async with aiohttp.ClientSession() as session:
        for item in item_list:
            if not item["image_urls"]:
                continue
            try:
                async with session.get(item["image_urls"][0]) as resp:
                    thumb_bytes[item["label"]] = await resp.read()
...
class TierList(commands.Cog):
    ...
    @app_commands.command(name="tierlist", description="Create a tier list - you'll be asked for items one at a time")
    @app_commands.describe(title="Title for the tier list")
    async def tierlist(self, interaction: discord.Interaction, title: str):
        ...
        item_list = await collect_named_options(interaction, "item", min_items=2, max_items=MAX_ITEMS)
        if len(item_list) < 2:
            await message.edit(content="Tier list creation cancelled — didn't get at least 2 items in time.")
            return

        thumb_bytes = await _fetch_thumbnails(item_list)
