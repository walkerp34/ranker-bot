...
async def get_average_position(post_id):
    """For blind ranking: average position (1=best) each item got across
    everyone who ranked it, plus how many people ranked it. Reuses
    tier_assignments with the position number stored as the 'tier' string."""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            """SELECT item, AVG(CAST(tier AS FLOAT)) AS avg_pos, COUNT(*) AS submissions
               FROM tier_assignments WHERE post_id = ? GROUP BY item""",
            (post_id,),
        ) as cursor:
            return await cursor.fetchall()
